...
export default function SocialInvitation({
  ...
  useEffect(() => {
    ...
    if (linkedIn && linkedIn === "y") {
      setSocialSuccess((oldSocials) => ({ ...oldSocials, linkedIn: "y" }));
    }
  }, []);
  ...
  return (
    <>
      <div className="z-10 mx-4 my-10 flex w-full max-w-2xl flex-col items-center justify-center gap-5 rounded bg-opacity-30 px-4 py-8 text-black shadow-lg backdrop-blur-3xl md:mx-14 md:p-16">
        ...
        socialSuccess.linkedIn ? (
          <button
            onClick={() => setShowSocialInvitation(false)}
            className="button-transition w-full bg-main py-4 text-center font-semibold tracking-widest text-white disabled:opacity-60 disabled:hover:cursor-not-allowed xs:w-52"
          >
            CONTINUE
          </button>
        ) : (
