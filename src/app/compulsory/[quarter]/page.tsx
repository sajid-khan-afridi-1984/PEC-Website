...
async function getData(quarter: string) {
  try {
    const SYLLABUS_API = process.env.SYLLABUS_API;
    console.log(
      "🚀 ~ file: page.tsx:10 ~ getData ~ SYLLABUS_API:",
      SYLLABUS_API
    );
    const res = await fetch(`${SYLLABUS_API}/api/compulsory/${quarter}`);
    const data = await res.json();
    console.log("🚀 ~ file: page.tsx:16 ~ getData ~ data:", data);
    if (!data) {
        ...
        <div className="relative m-auto mt-20 block w-[95%] justify-between gap-10 md:flex lg:w-[90%] xl:w-[1300px]">
          <div className="relative top-0 h-fit w-full flex-shrink-0 rounded-lg border border-zinc-300 md:sticky md:top-40 md:w-[270px] md:rounded-none md:border-none">
            <div className="h-fit rounded-lg bg-gray-50 p-3 ">
              <h2 className="leading-wider rounded-lg bg-main p-3 text-center text-sm font-semibold text-[#f1f1f1]">
                ...
                {tracks.map((val, index) => {
                  return (
                    <Link
                      ...
                      className="leading-2 rounded-lg bg-gray-50 px-3 py-2 text-sm tracking-wider text-zinc-800 transition-all hover:bg-main hover:text-white"
                    >
                      {val.name}
                    </Link>
                  );
