...
async function getData(track: string) {
  ...
  console.log("🚀 ~ file: page.tsx:10 ~ getData ~ res:", res);
  const apiresp = await res.json();
  console.log("🚀 ~ file: page.tsx:10 ~ getData ~ res:", apiresp);
                ...
                {["1", "2", "3"].map((val, index) => {
                  return (
                    <Link
                      ...
                      key={index}
                      className="w-1/3 rounded-lg bg-gray-50 p-3 text-center text-sm font-bold text-zinc-800 transition-all hover:bg-main hover:font-bold hover:text-[#f1f1f1]"
                    >
                      Q{val}
                    </Link>
                  );
